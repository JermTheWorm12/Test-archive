...
function renderMain(){
  ...
  document.querySelectorAll("input[data-allow-db]").forEach(el=>el.addEventListener("change",()=>{syncEditor();render()}));  const backdrop=document.getElementById("dialogBackdrop"); if(backdrop)backdrop.addEventListener("click",e=>{if(e.target===backdrop)closeDialog()});
}
...
function wireMainEvents(){document.querySelectorAll("[data-action]").forEach(el=>{el.addEventListener("click",async e=>{e.preventDefault();e.stopPropagation();const action=el.dataset.action;
  ...
  if(action==="backup-export")return triggerBackupDownload(); if(action==="start-self-destruct")return startSelfDestruct(); if(action==="confirm-self-destruct")return confirmSelfDestruct();
